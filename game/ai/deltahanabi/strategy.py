...
def get_next_plays(num_players, player_id, hands, board, turn, last_turn, full_deck, discard_pile, deck = None, hints = 0, all_plays = False):
    ...
    def simulate(player_id, hands, board, turn, last_turn, deck, hints, all_plays = False):
        if (last_turn is not None and turn > last_turn) or not any (c.playable(board) for h in hands.values() for c in h if c is not None):
            if all_plays:
        ...
        return plays[play]
    if all_plays:
        yield {p : s[1][0] for p, s in simulate(player_id, hands, board, turn, last_turn, deck, hints, True).items() if s >= 0}
    else:
...
class PublicKnowledge:
    ...
    def update_possible_cards(self):
        update_possible_cards([j for p in self.possible_cards.values() for j in p], lambda: set(self.ai.deck - Counter(self.ai.discard_pile) - self.known_cards()))
    ...
    def get_boards(self, player_id, last_turn, discard_pile):
        hands = {i : [next(iter(j)) if len(j) == 1 else None for j in p] for i, p in self.possible_cards.items()}
    ...
    def update_with_hint(self, hinter, hint, boards):
        ...
        my_board = None
        for i, b in zip(range(1, self.ai.num_players), boards):
...
class Strategy(BaseStrategy):
    ...
    def get_turn_action(self):
        ...
        if self.deck_size > 1 and self.hints < 8 and discard_value <= 0 and all(c not in u or len(p) == 1 for i, h in self.hands.items() for c, p in zip(h, self.pk.possible_cards[i])):
    ...
    def choose_hint(self, boards):
        ...
        new_pk = self.pk.clone()
        u = {}
        for i, b in zip(list(map(self.next_player_id, range(1, self.num_players))), boards):
    ...
    def choose_discard_for_hint(self, safe_discards, boards):
        ...
        number = sum(self.pk.encode_hand2(self.next_player_id(i), b, self.discard_pile, mod) for i, b in zip(range(1, self.num_players), boards)) % mod
    ...
    def get_best_play(self, pi):
        if self.deck_size <= 4:
            ...
            def simulate(player_id, hands, board, turn, last_turn, deck, hints):
                ...
                plays[None] = s
                if (last_turn is not None and turn > last_turn) or not any (c.playable(board) for c in itertools.chain(deck, *iter(hands.values())) if c is not None):
                    return plays
                if hints > 0:
                ...
                def simulate0(draw_card, deck, last_turn, mult = 1.0):
                    ...
                    if last_turn is None or hints == 0:
                        for i, c in discardable:
                            ...
                            hands[player_id][i] = c
                if len(deck) > 0:
                    for draw_card, mult in Counter(deck).items():
            ...
            plays = {}
            count = 0
            for h, mult in Counter(itertools.product(*(list(map(f, p)) for p in pi))).items():
                hands[self.id] = []
                deck = Counter(list(map(f, (self.deck - Counter(self.discard_pile)).elements()))) -  Counter(f(c) for h in self.hands.values() for c in h if c is not None)
                h_counter = Counter(h)
            ...
            hands = {i : [next(iter(j)) if len(j) == 1 else None for j in self.pk.possible_cards[i]] for i in self.hands.keys()}
    ...
    def get_best_discard(self, pi):
        ...
        visible = my_known + Counter(itertools.chain(*iter(self.hands.values())))
    ...
    def private_info(self):
        ...
        update_possible_cards(pi, lambda: set(self.deck - Counter(self.discard_pile) - Counter(itertools.chain(*iter(self.hands.values()))) - Counter({next(iter(j)) for j in pi if len(j) == 1})))
        return pi
