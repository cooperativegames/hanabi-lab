...
class Knowledge:
    ...
    def update_with_meaning(self, meaning):
        ...
        for (card_pos, cards) in meaning:
            self.possibilities[card_pos] = Counter(
                {card: v for (card, v) in p.items() if card in cards}
            )
    ...
    def update_with_hint(self, action):
        ...
        for (card_pos, p) in enumerate(self.possibilities):
            self.possibilities[card_pos] = Counter(
                {card: v for (card, v) in p.items() if card.matches_hint(action, card_pos)}
            )
    ...
    def update_combinations(self):
        ...
        if num_combinations <= self.MAX_COMBINATIONS:
            ...
            for combination in itertools.product(*[p for p in self.possibilities if len(p) > 0]):
                ...
                if all(possible_cards[card] >= v for (card, v) in Counter(combination).items()):
    ...
    def playable_probability(self, card_pos):
        ...
        return float(sum(v for (card, v) in p.items() if card.playable(self.strategy.board))) / sum(p.values())
